...
/*      if($stateParams != undefined){
      ...
      if(response.status == 200){
        if(response.data.length != 0){
          ...
          angular.forEach(response.data, function(value, key) {
            ...
            jobService.getJobDetailByJobId(invoice.jobs[0])
            .then(function(response){
                if(response.status == 200){
                  var item = {
                      invoice : invoice,
    ...
    $scope.approveInvoice = function(item){
      ...
      .then(function(response){
        if(response.status == 200){
          $scope.invoiceTobeApproved.contractTask = response.data;
          tripService.getTripStatesByTripId(item.invoice.trips[0])
          .then(function(response){
            if(response.status == 200){
              $scope.invoiceTobeApproved.tripStates = response.data;
              tripService.getTripDataByTripId(item.invoice.trips[0])
              .then(function(response){
                if(response.status == 200){
                  $scope.invoiceTobeApproved.tripData = response.data;
