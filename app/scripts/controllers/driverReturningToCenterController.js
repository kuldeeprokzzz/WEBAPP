angular.module('inloopAppApp')
  .controller('driverReturningToCenterController', function ($scope,$location,$interval,$stateParams,sharedProperties,completeModel,contractTaskService) {
...
var centerControlDiv = document.createElement('div');
 ...
 // Set CSS for the control border.
       ...
       // controlUI.title = 'Click to proceed to the Delivery Center';
        ...
        controlText.innerHTML = 'Returning to the Center';
    ...
    .then(function(response){
      if(response.status == 200){
        ...
        $scope.$on('$locationChangeStart', function(event, next, current){            
          if($location.path() == $scope.model.lastPath || $location.path() == '/driver/arrived' || $location.path() == '/driver/checkedIn' || $location.path() == '/driver/jobs/unassigned' || $location.path() == '/'){
          }else{
